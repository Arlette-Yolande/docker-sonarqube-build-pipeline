pipeline {
    ...
    environment {
        registry = "619975202394.dkr.ecr.us-east-1.amazonaws.com/sonarqubeapp"
    }
    stages {
        stage('Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Lion-Technology-Solutions/docker-sonarqube-build-pipeline.git']])
            }
        }
        ...
        stage ("Push to ECR") {
            steps {
                script {
                    ...
                    sh 'docker push 619975202394.dkr.ecr.us-east-1.amazonaws.com/sonarqubeapp:$BUILD_NUMBER'
